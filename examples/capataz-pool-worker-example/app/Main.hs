  ...
  in
    if Text.isPrefixOf "mailto:" link then
      ...
      case uriRegName <$> uriAuthority uri of
        Just domain ->
          Just $ Text.pack (uriScheme uri) <> "//" <> Text.pack domain <> link  -- Use domain from the req
...
webCrawler
  ...
  => Manager -> Text -> Int -> m0 (ProcessSpec m)
webCrawler manager startUrl workerCount = do
  ...
  let
    getNextLink = atomically $ readTBQueue urlLinkQueue
    addLink link = do
      atomically $ writeTBQueue urlLinkQueue link
...
main = do
  ...
  withLogFunc logOptions $ \logFunc -> runRIO logFunc $ do
    capataz <- forkCapataz "capataz-worker-pool-example"
