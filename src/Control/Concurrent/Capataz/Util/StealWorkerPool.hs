...
runPoolWorker
  :: (MonadUnliftIO m, MonadThrow m, Typeable a, Show a)
  => ((a -> m ()) -> m ())
  -> Text
  -> WorkerPoolArgs m a
  -> WorkerId
...
runPoolWorker requestWork poolWorkerName poolArgs poolWorkerId = do
  let WorkerPoolArgs { poolWorkerAction, poolWorkerTimeoutMicros } = poolArgs
  ...
  forever $ do
    ...
    result            <- timeout poolWorkerTimeoutMicros
      $ poolWorkerAction poolWorkerId poolWorkerPayload

    case result of
      Nothing -> throwM $ WorkerPoolTimeout
        { poolWorkerId
        , poolWorkerName
        , poolWorkerPayload
        }
...
buildWorkersFromPoolSpec requestWork poolArgs =
  let
    WorkerPoolArgs { poolWorkerNamePrefix, poolWorkerCount, poolWorkerOptions }
      = poolArgs
  in  [ workerSpec1 workerName
