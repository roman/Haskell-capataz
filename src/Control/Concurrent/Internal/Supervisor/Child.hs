...
childMain
  ...
  -> RestartCount
  -> IO Child
childMain (SupervisorEnv {supervisorQueue})
  ...
  childAsync <- async $ do
    ...
    atomically
      $ writeTQueue supervisorQueue (MonitorEvent result)

  return $ Child {childId, childAsync, childCreationTime, childSpec}

forkChild :: SupervisorEnv -> ChildSpec -> Maybe ChildId -> Maybe RestartCount -> IO ChildId
