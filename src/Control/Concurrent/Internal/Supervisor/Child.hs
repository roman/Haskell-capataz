...
childMain SupervisorEnv { supervisorQueue } childSpec@ChildSpec { childName, childAction, childOnError, childOnCompletion, childOnTermination } childId restartCount
  = do
    ...
    return Child {childId , childName, childAsync , childCreationTime , childSpec }
...
terminateChild terminationReason env@SupervisorEnv {supervisorName, supervisorId, notifyEvent} childId =
  removeChildFromMap env childId $ \Child {childName, childAsync} -> do
    ...
    notifyEvent (SupervisedChildTerminated { supervisorName
                                           , supervisorId
                                           , childId
                                           , childName
                                           , eventTime
    ...
    cancelWith childAsync (TerminateChildException {childId, terminationReason})
    wait childAsync

terminateChildren :: Text -> SupervisorEnv -> IO ()
terminateChildren terminationReason env@SupervisorEnv {supervisorName, supervisorId, supervisorChildMap, notifyEvent} = do
  eventTime <- getCurrentTime
  childrenIds <- HashMap.keys <$> readIORef supervisorChildMap

  notifyEvent (SupervisedChildrenTerminationStarted { supervisorName
                                                    , supervisorId
                                                    , terminationReason
                                                    , eventTime })
  forM_ childrenIds $ \childId ->
    terminateChild terminationReason env childId

  notifyEvent (SupervisedChildrenTerminationFinished { supervisorName
                                                     , supervisorId
                                                     , terminationReason
                                                     , eventTime })
