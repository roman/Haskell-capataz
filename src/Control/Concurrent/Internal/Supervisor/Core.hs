...
handleControlAction env controlAction =
...
handleSupervisorMessage env message =
  case message of
    ...
    MonitorEvent monitorEvent ->
      handleMonitorEvent env monitorEvent

runSupervisorLoop :: SupervisorEnv -> IO ()
runSupervisorLoop env@(SupervisorEnv { supervisorId
                                     , supervisorName
                                     , supervisorStatusVar
                                     , supervisorQueue
                                     , notifyEvent }) = do
...
forkSupervisor supervisorSpec = do
  supervisorRuntime <-
    buildSupervisorRuntime supervisorSpec

  let
    supervisorEnv =
      runtimeToEnv supervisorRuntime

  supervisorAsync   <-
    async $ runSupervisorLoop supervisorEnv
...
forkChild childSpec (Supervisor {supervisorEnv}) = do
  ...
  atomically $
    writeTQueue
      supervisorQueue
      (ControlAction $ ForkChild { childSpec
                                 , childRestartCount =
                                     0
                                 , returnChildId =
                                     putMVar childIdVar
